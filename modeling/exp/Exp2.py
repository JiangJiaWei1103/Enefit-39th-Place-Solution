...
class Exp(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        # Model blocks
        self.wave_block1 = _WaveBlock(8, 5, 32)
        self.wave_block2 = _WaveBlock(5, 32, 32)
        self.wave_block3 = _WaveBlock(3, 32, 64)
        self.wave_block4 = _WaveBlock(3, 128, 64)
    ...
    def forward(self, inputs: Dict[str, Tensor]) -> Tensor:
        ...
        x = self.gn2(x)
        x = self.wave_block3(x)  # (B, 64, T)

        x = torch.cat(
            [x[..., -48:], self.avg_pool(x[:, :32])[..., -48:], self.max_pool(x[:, 32:])[..., -48:]], axis=1
        )  # (B, 128, 48)
        x = self.wave_block4(x)[..., -24:]  # (B, 64, 24)

        x_p = torch.cat([x, tids, fwth], dim=1)  # (B, 83, 24)
        x_c = torch.cat([x, tids, fwth], dim=1)

        output_p = self.prod_head(x_p.transpose(1, 2))  # (B, 24, 1)
        output_c = self.cons_head(x_c.transpose(1, 2))  # (B, 24, 1)
