...
* Stick to BN.
* Consider only price features of the day before forecast day.
...
class Exp(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        self.price_enc = nn.GRU(4, 16, 1, batch_first=True)
        self.last_gru = nn.GRU(175, 128, 1, batch_first=True, bidirectional=True)
    ...
    def forward(self, inputs: Dict[str, Tensor]) -> Tensor:
        ...
        _, x_price = self.price_enc(price.transpose(1, 2))  # (1, B, 16)
        x_price = x_price.permute(1, 2, 0).expand(-1, -1, 24)
        x = torch.cat([x, x_bypass, tids, fwth, x_price], dim=1)  # (B, 175, 24)
