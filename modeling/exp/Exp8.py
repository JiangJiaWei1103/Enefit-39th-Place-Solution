...
* Cancel linear bypass.
* Use pc_cross bypass and pass to the p/c head directly.
...
class Exp(nn.Module):
    ...
    def forward(self, inputs: Dict[str, Tensor]) -> Tensor:
        ...
        x_p = torch.cat([x, x_bypass, tids, fwth], dim=1)  # (B, 159, 24)
        x_c = torch.cat([x, x_bypass, tids, fwth], dim=1)
        output_p = self.prod_head(x_p.transpose(1, 2))
        output_c = self.cons_head(x_c.transpose(1, 2))
